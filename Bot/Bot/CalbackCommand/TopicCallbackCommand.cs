...
namespace Bot.CalbackCommand
{
    public class TopicCallbackCommand : ICallbackCommand
    {
        ...
        public TopicCallbackCommand(ITelegramBotClient botClient, WorkFileBuilder workFileBuilder, FileStorageService fileStorage, SendMessageTeacher sendMessageTeacher)
        {
        ...
        public async Task ExecuteAsync(ITelegramBotClient botClient, CallbackQuery callback, CancellationToken cancellationToken)
        {
            ...
            if (filePath is not null)
            {
                ...
                await _sendMessageTeacher.SendMessageTeachers("тема урока заполнено неправильно");
                await botClient.SendMessage(
                    chatId: message.Chat.Id,
